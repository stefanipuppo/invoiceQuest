...
def main():
    ...
    directory = 'pdf_invoices'
     # Filtra APENAS arquivos PDF (ignore outros arquivos/pastas)
    files = [f for f in os.listdir(directory) 
             if f.lower().endswith('.pdf') and os.path.isfile(os.path.join(directory, f))]
    
    print(f"Arquivos encontrados: {files}")  # Debug (verifique no terminal)
    
    if not files:
        raise Exception("Nenhum PDF válido encontrado na pasta!")
    ...
    for file in files:
        try:
            ...
            inv_number_re_patter = r'INVOICE #(\d+)'
            inv_date_re_pattern = r'DATE: (\d{2}/\d{2}/\d{4})'

            match_number = re.search(inv_number_re_patter, pdf_text)
            ...
            execute_insert(cursor, "N/A", "N/A",file, e)
            db.commit()
